...
>
  ...
  <xsl:template match="/">
	...
	<xsl:if test="/gmi:MI_Metadata/gmd:identificationInfo/gmd:MD_DataIdentification/gmd:citation/gmd:CI_Citation/gmd:title/gco:CharacterString != ''">
	  <h2 class="arcmeta" style="padding-bottom: 10px; border-bottom: 1px solid #ccc;"><xsl:value-of select="/gmi:MI_Metadata/gmd:identificationInfo/gmd:MD_DataIdentification/gmd:citation/gmd:CI_Citation/gmd:title/gco:CharacterString" /></h2>
	</xsl:if>
	...
	<xsl:if test="/gmi:MI_Metadata/gmd:identificationInfo/gmd:MD_DataIdentification/gmd:abstract/gco:CharacterString != ''">
	  <DIV class="arcmeta_content abstract"><font style="font-weight:bold;">Abstract: </font>
       	  ...
       	  <xsl:choose>
       	    <xsl:when test="string-length(/gmi:MI_Metadata/gmd:identificationInfo/gmd:MD_DataIdentification/gmd:abstract/gco:CharacterString)>400">
       	    <!-- if length is longer than 400 (or whatever)-->
              <!-- Use spans to truncate the abstact and allow the remainder to be shown -->
       	      <div>
       	        <span style="display:none">
       	            <xsl:value-of select="/gmi:MI_Metadata/gmd:identificationInfo/gmd:MD_DataIdentification/gmd:abstract/gco:CharacterString" disable-output-escaping="yes"/>
       	            <a href="#" onclick="jQuery(this).parent().parent().children().toggle(); return false"> [Less]</a>
       	        </span>
       	        <span style="display:inline">
       	            <xsl:call-template name="trim-last-word">
                        <xsl:with-param name="in" select="substring(/gmi:MI_Metadata/gmd:identificationInfo/gmd:MD_DataIdentification/gmd:abstract/gco:CharacterString, 1, 400)" />
                    </xsl:call-template>
...
<!--
        <xsl:if test="/gmi:MI_Metadata/gmd:identificationInfo/gmd:MD_DataIdentification/gmd:purpose/gco:CharacterString != ''">
          <DIV class="arcmeta_content voice"><font style="font-weight:bold;">Purpose: </font>
          <xsl:value-of select="/gmi:MI_Metadata/gmd:identificationInfo/gmd:MD_DataIdentification/gmd:purpose/gco:CharacterString" />
          </DIV>
        </xsl:if>
-->
